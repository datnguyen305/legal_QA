...
class CHIMEGenerator(nn.Module):
    """CHIME-style hierarchical memory generator.

    The model keeps two memories: a context memory from question-aware passage
    states and an answer memory that is updated after each generated token.
    """
    ...
    def __init__(self, vocab_size: int, pad_token_id: int, bos_token_id: int, eos_token_id: int, hidden: int = 128, decoder_hidden: int = 256) -> None:
        ...
        self.context_encoder = nn.GRU(hidden, hidden // 2, batch_first=True, bidirectional=True)
        self.question_encoder = nn.GRU(hidden, hidden // 2, batch_first=True, bidirectional=True)
        self.mem_fuse = nn.Linear(hidden * 4, hidden)
        self.init = nn.Linear(hidden * 2, decoder_hidden)
        self.answer_memory_init = nn.Linear(hidden * 2, decoder_hidden)
        self.decoder = nn.GRUCell(hidden + hidden + decoder_hidden, decoder_hidden)
        self.answer_memory = nn.GRUCell(hidden + decoder_hidden, decoder_hidden)
        self.att_mem = nn.Linear(hidden, decoder_hidden, bias=False)
        self.att_dec = nn.Linear(decoder_hidden, decoder_hidden, bias=False)
        self.att_ans = nn.Linear(decoder_hidden, decoder_hidden, bias=False)
        self.att_v = nn.Linear(decoder_hidden, 1, bias=False)
        self.readout = nn.Linear(decoder_hidden * 2 + hidden * 2, decoder_hidden)
        self.vocab_proj = nn.Linear(decoder_hidden, vocab_size)
        self.copy_gate = nn.Linear(decoder_hidden * 2 + hidden * 2, 1)
    ...
    def forward(self, context_ids: Any, question_ids: Any, answer_ids: Any | None = None, max_answer_len: int | None = None, style_ids: Any | None = None) -> AbstractiveOutput:
        ...
        sim = torch.bmm(c, q.transpose(1, 2)).masked_fill(~q_mask.unsqueeze(1), -1e4)
        q_att = torch.bmm(torch.softmax(sim, dim=-1), q)
        context_memory = torch.tanh(self.mem_fuse(torch.cat([c, q_att, c - q_att, c * q_att], dim=-1)))
        q_vec = self._masked_mean(q, q_mask)
        ctx_vec = self._masked_mean(context_memory, c_mask)
        state = torch.tanh(self.init(torch.cat([ctx_vec, q_vec], dim=-1)))
        ans_mem = torch.tanh(self.answer_memory_init(torch.cat([ctx_vec, q_vec], dim=-1)))
        ...
        for t in range(steps):
            ...
            scores = self.att_v(
                torch.tanh(self.att_mem(context_memory) + self.att_dec(state).unsqueeze(1) + self.att_ans(ans_mem).unsqueeze(1))
            ).squeeze(-1)
            scores = scores.masked_fill(~c_mask, -1e4)
            attn = torch.softmax(scores, dim=-1)
            ctx = torch.bmm(attn.unsqueeze(1), context_memory).squeeze(1)
            state = self.decoder(torch.cat([prev_emb, ctx, ans_mem], dim=-1), state)
            ans_mem = self.answer_memory(torch.cat([prev_emb, state], dim=-1), ans_mem)
            readout = torch.tanh(self.readout(torch.cat([state, ans_mem, ctx, q_vec], dim=-1)))
            vocab_dist = torch.softmax(self.vocab_proj(readout), dim=-1)
            p_gen = torch.sigmoid(self.copy_gate(torch.cat([state, ans_mem, ctx, q_vec], dim=-1)))
